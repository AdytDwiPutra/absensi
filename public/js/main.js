(function ($) {
    $.fn.minimal_select = function (options) {
        ...
        function get_object_list(elem) {
            ...
            $(elem)
                ...
                .each(function (i, opt) {
                    ...
                    if ($(opt).is(":selected")) {
                        item =
                            '<li><a href="javascript:void(0);" class="selected" tabindex="-1" data-value="' +
                            option_val +
                            '">' +
                            option_text +
                            "</a></li>";
                        ...
                        item =
                            '<li><a href="javascript:void(0);" tabindex="-1" data-value="' +
                            option_val +
                            '">' +
                            option_text +
                            "</a></li>";
        ...
        function get_placeholder_text(elem, is_selected) {
            if (is_selected) {
                settings.placeholder_text = $(elem)
        ...
        function prepare_template(elem) {
            ...
            select_template =
                '<div id="' +
                ui_select_id +
                '" class="m-select" style="width: ' +
                settings.select_width +
                'px"><b><i></i></b><a href="javascript:void(0);" class="m-placeholder" title="' +
                settings.placeholder_text +
                '" style="width: ' +
                (settings.select_width - settings.arrow_padding) +
                'px">' +
                settings.placeholder_text +
                '</a><ul class="m-list" style="width: ' +
                settings.select_width +
                "px; max-height: " +
                settings.lit_max_height +
                "px; top:" +
                (settings.list_top + 30) +
                'px; opacity:0">' +
                object_list +
                "</ul></div>";
        ...
        function focus_up_option(ui_select_id) {
            var selected_index = $(ui_select_id)
                .find("a.selected")
                .parent("li")
                .index();
            if (selected_index != 0) {
        ...
        $("body").keydown(function (event) {
            if ($(".m-select").is(".active")) {
                ...
                var ui_select_id =
                    "#" + $(".m-placeholder:focus").parent("div").attr("id");
                show_hide_opt_list(ui_select_id);
            }
        });
    };
